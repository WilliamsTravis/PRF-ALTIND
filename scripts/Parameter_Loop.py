...
try:
    ...
    for i in tqdm(range(len(indexnames))):
        with np.load("data/indices/"+indexnames[i]+"_arrays.npz") as data:
            arrays = data.f.arr_0
            data.close()
        with np.load("data/indices/"+indexnames[i]+"_dates.npz") as data:
            names = data.f.arr_0
            data.close()
    ...
    arraydict ={indexnames[i]:arraydict[i] for i in range(len(indexnames))}
    gc.collect(2)
# Strike level
strikes = [.7,.75,.8,.85,.9]
...
for p in range(len(indexnames)):
    ...
    for ay in actuarialyears:
        ...
        for s in strikes:
            ...
            array = df[8]
            np.savez_compressed( # Save this as an array, too 
                     "E:\\data\\prfpayouts\\onlinedata\\AY"
            ...
            dates.columns = ["dates"]
            jarrays = np.array([a[1] for a in arrays])
            np.savez_compressed("E:\\data\\prfpayouts\\onlinedata\\AY"
                         +str(ay)+"\\"+str(int(s*100))+"\\frequencies\\"+indices[p]+"\\arrays", jarrays)
            dates.to_csv("E:\\data\\prfpayouts\\onlinedata\\AY"
