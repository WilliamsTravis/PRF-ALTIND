...
app.layout = html.Div([
               ...
               # Hidden DIV to store the final id used by the bar charts
...
def submitSignal(clicks, index_choice, actuarial_year, year_slider,
    ...
    '''
    Store data in the cache and hide the signal to activate it in the hidden
    div.
    '''
...
def update_trendinfo(signal, clickData):
    ...
    # Description for Info Button:
    trendinfo = ("This bar chart shows the average monthly amount of payout " +
                 "potential at grid cell #" + targetid + " using the " +
                 "experimental index insurance program with a 50% allocation" +
                 " of total protection to each insurance interval from a " +
                 "hypothetical policy on a 500 acre ranch at the " +
                 str(int(strike_level*100)) + "% Strike Level and 100% "
                 "productivity level.")
    return trendinfo
...
def update_seriesinfo(signal, clickData):
    ...
    # Description for Info Button:
    seriesinfo = ("This is a time series of potential payouts that would " +
                  "have been received between " + str(year1) + " and " +
                  str(year2) + " at grid cell #" + targetid + " using the " +
                  "experimental index insurance program with a 50% " +
                  "allocation of total protection to each insurance " +
                  "interval from a hypothetical policy on a 500 acre ranch " +
                  "at the " + str(int(strike_level*100)) + "% Strike Level " +
                  "and 100% productivity level.")
...
@app.callback(Output('targetid_store', 'children'),  # <----------------------- This can be skipped using new features
...
def makeMap(signal, maptype):
    ...
    rt = return_type
    if rt == "premiums" or rt == "indemnities" or rt == "nets":
        datasymbol = "$"
    else:
        datasymbol = ""
    ...
    pdf['printdata'] = ("GRID #: " + pdf['grid'] +
                        "<br>Data: " + pdf['data'].apply(str))
...
# Create the scattermapbox object
    ...
    layout['title'] = ("<b>" + indexnames.get(signal[0]) + "<br>" + calc +
                       returndict.get(return_type) + " | " +
                       str(signal[2][0]) + " to " + str(signal[2][1]) +
                       " | " + str(int(strike_level*100)) +
                       "% Strike Level</b>")
...
def makeTrendBar(clickData, signal, targetid):
    ...
    print("############## Trend Map Target ID: " + str(targetid) + ", type: " +
          str(type(targetid)) + " #####################")
    ...
    if max(averages) < .75*maxy:
        ...
        yaxis = dict(title=trenddict.get(return_type),
                     autorange=False,
                     range=[ylow, max(averages)*1.2],
                     type='linear')
        annotation = dict(text="<b>(rescaled)</b>",
                          x=0.95,
                          y=0.95,
                          font=dict(size=17,
                                    color="#000000"),
...
def makeSeries(clickData, signal, targetid):
    ...
    # Filter by year range
    df = [d for d in df if
          int(d[0][-6:-2]) >= date1 and int(d[0][-6:-2]) <= date2]
    ...
    if return_type == "pcfs" or return_type == "lossratios":
        valuesum = ("<b>Total: {:,}".format(round(np.nanmean(values), 4)) +
                    "</b>")
    ...
    if max(values) < 1.5*maxy:
        yaxis = dict(title=seriesdict.get(return_type),
                     autorange=False,
                     range=[ylow, 1.5*maxy],
                     type='linear')
        annotation = dict(text=valuesum,
                          x=0.95,
                          y=0.95,
                          font=dict(size=17,
                                    color="#000000"),
        ...
        yaxis = dict(title=seriesdict.get(return_type),
                     ...
                     type='linear')
        annotation = dict(text=valuesum + "<br><b> (rescaled)</b>",
                          x=0.95,
                          y=0.95,
                          font=dict(size=17,
                                    color="#000000"),
    ...
    incities = (cities_df['NAME'][cities_df['grid'] == targetid] + ", " +
                cities_df['STATE'][cities_df['grid'] == targetid])
